...
def run(cfg, conn):
    ...
    rows = conn.execute(
        ...
        "kills_a, kills_b FROM games ORDER BY date, game_id").fetchall()
    if not rows:
        sys.exit("banco vazio — rode python -m src.ingest_oracle")
    cut = (datetime.fromisoformat(rows[0][1])
           + timedelta(days=int(bt["burnin_days"]))).isoformat(sep=" ")
    ...
    for gid, d, league, a, b, winner, ka, kb in rows:
        ...
        p_banda = win_probability(banda.get(a, seed_default),
                                  banda.get(b, seed_default))
        y = 0 if winner == "a" else 1       # classe 0 = time A vence
        medir = (d >= cut and n_games_seen[a] >= min_games
                 and n_games_seen[b] >= min_games)
        if medir:
        ...
        if ka is not None and kb is not None:
            ...
            if medir and len(lt) >= min_lg:
                mu_lg = st.mean(lt)
                sd_lg = st.stdev(lt)
                kpg_a = (st.mean(team_kills[a][-40:])
                         if len(team_kills[a]) >= min_games else mu_lg / 2)
                kpg_b = (st.mean(team_kills[b][-40:])
                         if len(team_kills[b]) >= min_games else mu_lg / 2)
                for ln in lines:
...
def main():
    ...
    acc_m = st.mean(1 if (p[0] >= 0.5) == (y == 0) else 0
                    for p, y in zip(r["probs_m"], r["outs"]))
    dm_stat, dm_p = diebold_mariano(r["loss_m"], r["loss_b"])[:2]
    melhor = br_m < br_b
    h1_ok = melhor and dm_p < 0.05
    print(f"\nH1-LOL (vencedor): Brier modelo {br_m:.4f} vs banda {br_b:.4f} "
          f"(coin-flip 0.5000)")
    print(f"  log-loss {ll_m:.4f} vs {ll_b:.4f} | acerto {acc_m:.1%} | "
          f"DM stat {dm_stat:+.2f} p={dm_p:.4f}")
    print(f"  VEREDITO H1-LOL: {'COMPROVADA' if h1_ok else 'REFUTADA'} "
    ...
    summary["h1"] = {"brier_modelo": round(br_m, 4), "brier_banda": round(br_b, 4),
                     "logloss_modelo": round(ll_m, 4), "logloss_banda": round(ll_b, 4),
                     "acerto": round(acc_m, 4), "dm_stat": round(dm_stat, 3),
                     "dm_p": round(dm_p, 5),
    ...
    for ln, ev in r["kills"].items():
        ...
        print(f"  linha {ln}: n={len(ev['y'])} | Brier modelo {bm:.4f} vs "
              f"liga {bb:.4f} | DM p={pval:.4f} "
              f"{'← modelo vence' if win and pval < 0.05 else ''}")
        h2[str(ln)] = {"n": len(ev["y"]), "brier_modelo": round(bm, 4),
                       "brier_liga": round(bb, 4), "dm_p": round(pval, 5)}
    h2_ok = ok_lines >= 2      # maioria das 3 linhas
    print(f"  VEREDITO H2-LOL: {'COMPROVADA' if h2_ok else 'REFUTADA'} "
          f"({ok_lines}/3 linhas com Brier menor e DM p<0.05)")
    summary["h2"] = {"linhas": h2, "linhas_vencidas": ok_lines,
                     "verdict": "COMPROVADA" if h2_ok else "REFUTADA"}
    ...
    (data / "team_stats_pesquisa.json").write_text(
        ...
        + "\n", encoding="utf-8")
    ligas = {lg: {"media_total_kills": round(st.mean(v), 2),
                  "sigma": round(st.stdev(v), 2), "n": len(v)}
             for lg, v in r["lg_totals"].items() if len(v) >= 30}
    (data / "calibration.json").write_text(
        json.dumps(ligas, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
