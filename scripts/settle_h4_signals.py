...
def _dt(value):
    ...
    if value.tzinfo is None:
        raise H4Error("timestamp de resultado sem timezone")
    return value
...
def settle(signals: Path, results: Path) -> int:
    ...
    for row in rows:
        ...
        available = _dt(result.get("result_available_at"))
        start = _dt(row["event_start_at"])
        if available < start:
            raise H4Error("resultado disponível antes do evento")
        if row.get("settlement_status") == "OFFICIAL":
            if row.get("result") != result["result"]:
                raise H4Error("correção de resultado exige nova coorte")
            continue
        row.update(
            {
                "result": result["result"],
                "result_available_at": available.isoformat(),
                "result_source": result["source"],
                "settlement_status": "OFFICIAL",
...
def main(argv=None):
    ...
    a = p.parse_args(argv)
    try:
        print(json.dumps({"settled": settle(a.signals, a.results)}))
    except (H4Error, OSError, ValueError, json.JSONDecodeError) as e:
