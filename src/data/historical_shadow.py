...
def report(conn):
 total=conn.execute('select count(*) from matches').fetchone()[0]; leagues=conn.execute('select count(distinct league) from matches').fetchone()[0]; teams=conn.execute('select count(distinct team_id) from (select team_a_id team_id from matches union select team_b_id from matches)').fetchone()[0]
 return {'series':total,'leagues':leagues,'teams':teams,'unit':'series','promotion_safe':False,'reason':'modelo LoL e calibrado por mapa; exige reconciliacao de series para mapas'}
