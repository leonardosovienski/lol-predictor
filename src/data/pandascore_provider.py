...
class PandaScoreProvider:
    ...
    def _headers(self):
        ...
        return {"Authorization": f"Bearer {self.token}",
                "User-Agent": "lol-predictor-source-audit/1.0"}
    ...
    def iter_past(self, *, from_date: str, to_date: str, per_page: int = 100,
        ...
        while max_pages is None or page <= max_pages:
            ...
            for row in _normalize_past(payload): yield row
...
def _normalize_past(payload):
    for match in payload:
        ...
        try:
            ...
            ids=[int(x["id"]) for x in teams]; names=[x["name"] for x in teams]
            values=[int(scores[x]) for x in ids]
            if started.tzinfo is None or values[0]==values[1]: raise ValueError
        except (KeyError,TypeError,ValueError): continue
        yield {"source":"pandascore","source_event_id":str(match["id"]),
          "started_at":started.astimezone(timezone.utc).isoformat(timespec="seconds"),
          "team_a_id":ids[0],"team_b_id":ids[1],"team_a":names[0],"team_b":names[1],
          "score_a":values[0],"score_b":values[1],"winner":"a" if values[0]>values[1] else "b",
          "format":match.get("number_of_games"),"league":(match.get("league") or {}).get("name"),
          "serie":(match.get("serie") or {}).get("full_name"),"unit":"series","shadow_only":True}
