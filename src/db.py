...
UPSERT = ("INSERT OR REPLACE INTO games (game_id, date, league, split, game, "
...
def upsert_games(conn, rows: list[dict]) -> int:
    cur = conn.executemany(UPSERT, [
        (r["game_id"], r["date"], r.get("league"), r.get("split"),
         r.get("game"), r["team_a"], r["team_b"], r["winner"],
         r.get("kills_a"), r.get("kills_b"), r.get("completeness"))
        for r in rows])
    conn.commit()
    return cur.rowcount
