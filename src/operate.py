...
def main(argv=None):
    ...
    p.add_argument("--format", default="bo3", choices=["bo1", "bo3", "bo5"])
    ...
    a = p.parse_args(argv)
    pred = run(a.team_a, a.team_b, fmt=a.format)
    if a.selection not in (a.team_a, a.team_b):
        p.error("--selection deve ser exatamente team_a ou team_b")
    ...
    print(json.dumps({"gate": go_gate(), "bet": bet}, ensure_ascii=False, indent=2))
    return 0
