...
def go_gate(path: str | Path | None = None) -> dict:
    ...
    ready = (
        data.get("verdict") == "GATE_PASSED_FOR_PROSPECTIVE_SHADOW"
    ...
    return {
        "decision": "NO-GO",
        "reason": "shadow aprovado; operação financeira continua desabilitada"
        if ready
        else "amostra financeira/gate insuficiente",
    }
...
def record_bet(
    ...
    **metadata,
    ...
    row = {
        ...
        "prob_model": prob_model,
        "decimal_odds": decimal_odds,
        "edge": round(prob_model * decimal_odds - 1, 6),
        "bankroll": bankroll,
        "stake": stake,
        "real": real,
        **metadata,
